...
Same as simulation4, except progresses automatically without user hitting <enter>. better for large simulations, so automatic dimensions are set larger. note that it can be harder to watch direction as disease progresses, so better for simply seeing end product. fixed list index bug so that probelm is solved by printing '0' instead of breaking
