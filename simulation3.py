...
Similar to simulation 2, except more user-friendly, fixes bug with b-type transmission, allows for more generations (but it restarts at printing '0'). fixes bug with end display. informs user of more variables at the end (people who werent vaccinated who did not contract the disease. note: people who do not contract disease were not exposed on four direct sides (type A) or all eight points of contact (type B). no other variables are used.)
Takes into accound only: innoculated vs not innoculated
% of people infected when in contact with infected person = 100% 

NOTE:
bug that appears occasionally that has not been fixed:

Traceback (most recent call last):
  File "simulation3.py", line 248, in <module>
    main()
  File "simulation3.py", line 242, in main
    finished = evolve(board, width, height, generation, genList, transmissionType)
  File "simulation3.py", line 159, in evolve
    board = infectNew(board, newInfected, generationStr)
  File "simulation3.py", line 143, in infectNew
...
def endBoard(board, width, height, generation):
...
type 'a' for transmission type a or 'b' for transmission type b and hit <Enter>'''
    ...
    endBoard(board, width, height, generation)
