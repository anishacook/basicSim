...
def checkNeighbors(board,width,height,cellX,cellY, transmissionType): #cellX, cellY are coordinates of infected cell. MUST DO THIS FOR EVERY INFECTED CELL
    ...
    if transmissionType == 'a':
        if (0 <= cellY-1 < height):
            if board[cellX][cellY-1] == colored('+','white'):
                newInfected.append([cellX, cellY-1])
        if (0 <= cellY+1 < height):
            if board[cellX][cellY+1] == colored('+','white'):
                newInfected.append([cellX, cellY+1])
        if (0 <= cellX-1 < width):
            if board[cellX-1][cellY] == colored('+','white'):
        ...
        if (0 <= cellY-1 < height) and (0 <= cellX-1 < width):
            if board[cellX-1][cellY-1] == colored('+','white'):
        ...
        for j in range(height):
            if board[i][j] != colored('.','green'):  
                if board[i][j] != colored('+','white'):
                    ...
                    board [i][j] = colored('.','red')
                else:
                    board[i][j] = colored('O', 'green')
                    numberOkaySusceptibles += 1.0
            elif board[i][j] == colored('.','green'): 
                board[i][j] = colored('O', 'green')
                innoculatedPeople += 1.0
    board[patientZero[0]][patientZero[1]] = colored('O', 'white')
    print '\n\nYOUR POPULATION (where green is healthy person and red is an infected person and white is the infected patient zero):\n'
    printBoard(board, width, height)
